...
class Initialize:
	...
	def TableValToUserInput(self,itemsString,values):
		...
		for i,a in enumerate(lst):
			...
			j =forms(obj)
			if type(obj)==Gtk.Entry or type(obj)==Gtk.CheckButton or type(obj)==Gtk.Switch:
				j.Text=values[i]
			if str(type(obj))=="<class 'richEntry.Handler'>":
				obj.Text=values[i]
				continue
			#import pdb;pdb.set_trace();
			if type(obj.obj)==Gtk.ComboBoxText:
				obj.Text=values[i]
	...
	def CboNameClick(self,*args):
		try:
			...
			if total=="0":
				print ("Out of Stock " + self.txtName_Sales.Text)
		except:
			a=0
