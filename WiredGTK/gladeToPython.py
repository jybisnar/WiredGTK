...
class Handler(object):
    ...
    def GTKForms(self):
        ...
        self.create("{'Visible': 'True', 'Text': 'root', 'Enable': 'True', 'Alignment': '', 'Tag': '', 'Font': '', 'BackColor': '(1,1,1,1)', 'Left': '131', 'ForeColor': '(0,0,0,1)', 'Height': '30', 'Width': '170', 'Top': '107', 'Picture': '', 'Name': 'txtuser', 'Events': '[]', 'ParentsType': ''}","Entry","usercontrol","[]")
        self.create("{'Visible': 'True', 'Text': 'wta', 'Enable': 'True', 'Alignment': '', 'Tag': '', 'Font': '', 'BackColor': '(1,1,1,1)', 'Left': '131', 'ForeColor': '(0,0,0,1)', 'Height': '30', 'Width': '170', 'Top': '81', 'Picture': '', 'Name': 'txtdatabase', 'Events': '[]', 'ParentsType': ''}","Entry","usercontrol","[]")
        self.create("{'Visible': 'True', 'Text': '', 'Enable': 'True', 'Alignment': '', 'Tag': '', 'Font': '', 'BackColor': '(1,1,1,1)', 'Left': '131', 'ForeColor': '(0,0,0,1)', 'Height': '30', 'Width': '170', 'Top': '139', 'Picture': '', 'Name': 'txtpassword', 'Events': '[]', 'ParentsType': ''}","Entry","usercontrol","[]")
    ...
    def Button1_clicked(self,e):
        fnameGlade=dlgFile("Select db file",types=['*.txt'])  
        if type(fnameGlade)==int:
            return
        if fnameGlade.find(".dbbuilder")==-1:
                msgbox(None, "Select '.dbbuilder' files", "Glade To Code") 
        else:	
                b=CreateFormsAndMysql(fnameGlade,_host=self.txthost.Text,_user=self.txtuser.Text,_db=self.txtdatabase.Text,_port=self.txtport.Text,_passwd= self.txtpassword.Text)
                b.Generate()
...
class CreateFormsAndMysql:
    def __init__(self,filename,_host,_user,_db,_port,_passwd):
            ...
            try:
                if self.sqlinserttable!="":
                    try:
                        ...
                        pass
                    try:
                        sql="ALTER TABLE autonumber ADD COLUMN " + self.sqlinserttable + " AFTER ignores"
    ...
    def Generate(self):
        ...
        for table in self.table:
            self.insertcode="\t\tself.datagrid.ExecuteNonQuerry(self.db,\"INSERT INTO `"+self.dbname+"`.`"+ table[0].table + "` "
            self.updatecode="self.datagrid.ExecuteNonQuerry(self.db,\"UPDATE `"+self.dbname+"`.`"+ table[0].table + "` SET "
            self.deletecode="self.datagrid.ExecuteNonQuerry(self.db,\"DELETE FROM `"+self.dbname+"`.`"+ table[0].table + "` "
            self.updatecode1=""
            self.updatecode2=""
            self.insertcode1="("
            self.insertcode2="("
            ...
            self.datagridCode+=    '\t\tself.db = MySQLdb.connect(host="localhost",user="root",db="'+self.dbname+'",port=3306)\r'
            self.datagridCode+=    '\t\tself.datagrid.DataSource(self.db,"select * from ' +   table[0].table + '")\r'
            ...
            self.updatecode=self.updatecode+self.updatecode1[:-1]+" WHERE "  + wheresql +  "\" % "+self.insertcode3[:-1]+"," + wheresql2 + "))"  +'\r\t\tself.datagrid.DataSource(self.db,"select * from ' +   table[0].table + '")'
            self.deletecode=self.deletecode+" WHERE " + deletewhere +  "\" % " + wheresql2 +")" +'\r\t\tself.datagrid.DataSource(self.db,"select * from ' +   table[0].table + '")'
            ...
            fglade= GetPath(self.filename)+"\\frm" + currTable +'.glade'
            ...
            self.updateGrid='\t\tself.datagrid.DataSource(self.db,"select * from ' +   table[0].table + '")\r'
    ...
    def __init__(self,class_handle,flag=True,filename="",INSERTCODE="",POPULATE="",updatecode="",deletecode=""):
       ...
       # appendInit(pythoncode,'self.treeviewcolumn2 = builder.get_object("treeviewcolumn2")')    
        ...
        if pycodeExist==False:
            ...
            prop['Height']=str(130)    
            prop['Tag']='Activex'
            GTKForms+=('\t\tself.create("' + str(prop) + '","' + 'datagridwta' + '","usercontrol","'+  "[['row_activated', 'self,value']]" +'")\n')
        ...
        tableClickCode=	'\tdef datagrid_row_activated(self,value):\r'
        tableClickCode+='\t\tself.dbUtils.TableValToUserInput("'+ table +'",value)\r'
