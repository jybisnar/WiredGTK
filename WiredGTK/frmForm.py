...
class generateCode(object):
	...
	def Generate(self,addedobj,deleted,gtkclass,plugin,currentform=None):
		...
		for a in addedobj.lst:
			...
			#cbuf='\t\tself.create("%s","%s","%s","%s");;;self.%s=self.%s;\r'%(str(prop).replace('\"','\\\"'),gtktype,parent,str(event),a.Name,a.Name)
			if a.Tag=="Activex":
				cbuf='\t\timport %s\r\t\tself.%s=%s.Handler\r'%(a.types,a.Name,a.types)
			else:
				cbuf='\t\tself.%s=forms\r'%(a.Name)			
...
class Handler:
	def __init__(self,gtkwta,filename):
		...
		try:
			...
			resizeH=float(screensize[1]/768.0)
		if screensize[1]<768:
			self.layoutCodeW.Height-=200
			self.scrolledwindow3W.Height-=200
			self.layout1W.Height-=200			
	def CheckWing(self):
	...
	def selectedFunc(self,val=None):
		...
		if self.gtkcodeflag==False and self.editor[1].usercontrol.Visible==False:
		...
		if self.editor[1].usercontrol.Visible==True:#if self.layoutCodeW.Visible==True:
			...
			if len(self.textview_current_str_ptr[0])==1 and self.textview_current_str_ptr[0].isalpha()==False:
				...
				#jedi take over
				target=self.textview_current_str_ptr[0];
				x=target[-1::-1].find(".")
				
				if x==-1:x=len(target)
				
				target=target[len(target)-x:]
				buffers.delete(buffers.get_iter_at_offset( self.textview_current_str_ptr[1]+len(self.textview_current_str_ptr[0])-x),buffers.get_iter_at_offset( self.textview_current_str_ptr[2]))	
	...
	def textview1_key_release_event_cb(self,*args):
		...
		if self.intel.usercontrol.Visible ==True:
			if args[1].get_keyval()[1]==UP: 
				return True
			if args[1].get_keyval()[1]==DOWN:
				return True
			if args[1].get_keycode()[1]==9 and args[1].get_keyval()[1]==TAB or args[1].get_keycode()[1]==23 and args[1].get_keyval()[1]==65289:
				return True
		...
		if self.helptriggered:   
			...
			try:
				if self.lasttypedword==self.textview_current_str_ptr[0]:#prevent repeated call to intellisense if same word
					return
			except:
				pass
			self.lasttypedword=self.textview_current_str_ptr[0]
			...
			if len(self.lstprev)==0:
				...
				'''def busymouse():
					...
					for a in range(3):
						self.layout2.get_window().set_cursor(watch_cursor)
						refreshGTK()	
						delay(3)
				'''
				...
				sys_path =self.prop.jedi.get_default_environment().get_sys_path()
				path=GetPath(self.filename)[:-1]
				script=self.prop.jedi.Script(source=self.editor[1].Text,sys_path=sys_path + [path])
				cc=getvardefinition(self.textview_current_str_ptr[0], x)
				
				'''
				a=filter(lambda suggest:suggest.name=='abs', suggest)
				a=list(a)
				a.params
				'''
				def extractlst(lst,x):
				...
				lst=deepcopy(self.lstprev)#jedi
			print("jedi finish")	
			#busymouse()	
			#self.layout2.get_window().set_cursor(watch_cursor)	
			nword=self.textview_current_str_ptr[0]	
			lst=getavailableWord(lst,nword)	
			lst=filter(lambda lst:lst!='', lst)#filter empty string in array
			lst=list(lst)
			if len(lst)==0:
				self.helptriggered=False
			if len(lst)!=0:
				
				strs=targetobject(self.textview_current_str_ptr[0])
				if len(strs)==2 and strs[1]!="":
					self.intel.ShowIntellisense(lst,strs[1])                
				else:
					self.intel.ShowIntellisense(lst)                
			else:
				self.intel.usercontrol.Visible=False
		self.prop.keyDebug(args[0],args[1])
		#self.layout2.get_window().set_cursor(None)
	...
	def textview1_key_press_event_cb(self,*args):
		...
		if args[1].get_keyval()[1]>0x110000:#prevent error eg capslock
			return True
		if self.intel.usercontrol.Visible ==True:
			if args[1].get_keyval()[1]==UP:
				self.intel.ScrollUp()
		...
		if self.currlinepos!=self.currlineposprev or chr(args[1].get_keyval()[1])==".":
			self.helptriggered=False
	...
	def btncode_clicked_cb(self,*args):
		...
		if self.editor[1].Text.find('#	Generated By WiredGTK for Python: by Rocky Nuarin')==-1 and self.code!="" and self.editor[1].Text!="":
		...
		if self.code=="":
			...
			if  self.filename.find('UnTitled')!=-1:
				...
				self.prop.btnSaveAs_clickedX(None)
		if self.filename.find('UnTitled')!=-1:
			self.prop.close_script()
			return
		if FileExist(self.filename):
			if GetFileStr(self.filename).find('#	Generated By WiredGTK for Python: by Rocky Nuarin')==-1:
				return
		g=generateCode(self.filename,self.code,self.lastevent)
		g.Generate(self.addedobj,self.deleted,self.gtkClass,self.plugin,self)
		self.lastevent=''
