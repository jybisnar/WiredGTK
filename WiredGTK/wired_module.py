...
def ExitApplication():
	Gtk.main_quit()
def initUI(self,param,w=1366,h=768,title="WiredGTKV1.0",controlbox=True,startpos=(200,200),timeoutdestroy=-1):
	self.caller=None
	self.param=None
	self.wiredevents={}
	if len(param)!=0:
...
def createWidget(self,prop,control,parent,event=[]):
	...
	if prop['ParentsType']=="Layout" and prop['Tag']!='Activex':
		...
		if prop['Tag']=='Activex':
			cmd="self._%s.put(self.%s.Widget(),%s,%s)"%(parent,name,prop["Left"],prop["Top"])
		else:
			cmd="self._%s.put(self._%s,%s,%s)"%(parent,name,prop["Left"],prop["Top"])#widget that has no parent,put it to our usercontrol
...
class forms(object):
	...
	def Text(self):
		if type(self.obj)==Gtk.Entry or type(self.obj)==Gtk.Label:
			...
			return self.obj.set_label()
		elif type(self.obj)==Gtk.TextView or type(self.obj)==GtkSource.View:#our january starts at 1
	...
	def Text(self,value):
		...
		if type(self.obj)==Gtk.Button:
		...
		elif type(self.obj)==Gtk.LinkButton or type(self.obj)==Gtk.CheckButton or type(self.obj)==Gtk.RadioButton:
			return self.obj.set_label(value)
		elif type(self.obj)==Gtk.Calendar:#our january starts at 1
			try:
				value=value.split('-')
			...
			try:
				...
				value=value
		elif type(self.obj)==Gtk.ComboBoxText:#produces combo change event problem 
...
#			self.obj.set_from_file (fname)	
	...
	def BackColor(self,*value):		#value=0.0, 1.0, 0.0, 1.0
		if type(self.obj)==Gtk.TextView or type(self.obj)==Gtk.Entry or type(self.obj)==GtkSource.View  :return
