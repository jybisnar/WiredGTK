...
class tokenKO:
	...
	def Start(self,fileOrString):
		...
		def SaveFileStrX(fname,str_to_write):
			...
			fo.close()		
		SaveFileStrX(GetPath(fileOrString) +'/'+ GetFilenameNoEXT(fileOrString)+'.bas',_code.encode())
		SaveFileStrX(GetPath(fileOrString) +'/_'+ GetFilenameNoEXT(fileOrString)+'.bas',code)
		a=0			
		f.close()	
